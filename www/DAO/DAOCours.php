...
function selectWithGroupeEtudiantMatiereCours($id_filiere, $libelle_groupe, $id_matiere) {
    ...
    $bd = getConnexion();
    $rqt = "SELECT C.date_debut FROM cours C JOIN matiere M ON C.id_matiere = M.id_matiere WHERE C.id_filiere = :id_filiere AND C.libelle_groupe = :libelle_groupe AND id_matiere = :id_matiere ";
    $stmt = $bd->prepare($rqt);
    $stmt->bindParam(":id_filiere", $id_filiere);
    $stmt->bindParam(":libelle_groupe", $libelle_groupe);
    $stmt->bindParam(":id_matiere", $id_matiere);
